...
func TestSubscribeRequest_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: apperr.ErrInvalidEmailFormat,
		},
		{
			name: "returns invalid repository format when repo is invalid",
			request: SubscribeRequest{
				Email: "user@example.com",
				Repo:  "owner",
			},
			wantErr: apperr.ErrInvalidRepositoryFormat,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertValidationErrorIs(t, err, tt.wantErr)
		})
	}
}
...
func assertValidationErrorIs(t *testing.T, got error, want error) {
	t.Helper()
