...
func (s *RepositorySuite) TestRepositoryStore_CreateAndGetByName() {
	created, err := s.repositoryStore.Create(s.ctx, "owner/repo", "v1.0.0")
	...
	got, err := s.repositoryStore.GetByName(s.ctx, "owner/repo")
...
func (s *RepositorySuite) TestRepositoryStore_UpdateTag() {
	...
	err = s.repositoryStore.UpdateTag(s.ctx, created.ID, "v2.0.0")
	s.Require().NoError(err, "update repository tag")
	updated, err := s.repositoryStore.GetByName(s.ctx, "owner/repo")
...
func (s *RepositorySuite) TestRepositoryStore_GetAll() {
	_, err := s.repositoryStore.Create(s.ctx, "owner/repo", "v1.0.0")
	s.Require().NoError(err, "create repository")

	all, err := s.repositoryStore.GetAll(s.ctx)
...
func (s *RepositorySuite) TestRepositoryStore_GetByNameReturnsNotFound() {
	_, err := s.repositoryStore.GetByName(s.ctx, "missing/repo")
