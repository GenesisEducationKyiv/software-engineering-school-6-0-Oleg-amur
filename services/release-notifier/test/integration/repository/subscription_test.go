...
func (s *RepositorySuite) TestSubscriptionStore_CreateRejectsDuplicateSubscriberRepository() {
	...
	_, err := s.subscriptionStore.Create(s.ctx, subscription.subscriber.ID, subscription.repository.ID, "token-2")
...
func (s *RepositorySuite) TestSubscriptionStore_GetActiveByRepoIDReturnsOnlyActiveSubscribers() {
	...
	_, err = s.subscriptionStore.Create(s.ctx, pendingSubscriber.ID, activeSubscription.repository.ID, "pending-token")
	s.Require().NoError(err, "create pending subscription")

	err = s.subscriptionStore.Activate(s.ctx, activeSubscription.token)
	s.Require().NoError(err, "activate subscription")

	active, err := s.subscriptionStore.GetActiveByRepositoryID(s.ctx, activeSubscription.repository.ID)
